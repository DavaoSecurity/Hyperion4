#!/bin/env bash
# dns data gathering with output to xml, text and possibly html
clear
...
echo " Summary DNS Tests txt " | mail -s "DNS txt Reports for "$1" " -A MyDNStxt.zip $2
echo " Summary DNS Tests XML " | mail -s "DNS xml and HTML Reports for "$1" " -A MyDNSDiscXML.zip $2

rm *.zip *.html nd*.txt dns0.txt nslookmxup.txt hostmx.txt nslookmx.txt digmx.txt spfr.txt dmarcr.txt
rm nslookupmx.txt hostls.txt digns.txt EDIM.txt *.xml
